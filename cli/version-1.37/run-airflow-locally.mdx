...
Alternatively, you can run `astro dev stop` to stop your Docker containers without restarting, then run `astro dev start` when you want to restart.
...
`astro dev run` is the equivalent of running `docker exec` in local Airflow containers.
...
To make requests to the Airflow REST API in a Deployment on Astro, see [Airflow API](/astro/airflow-api).

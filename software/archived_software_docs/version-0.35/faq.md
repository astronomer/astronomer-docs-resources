...
The default method for installing and upgrading Astronomer Software is using the Astronomer Helm chart located at `https://helm.astronomer.io`. See the [Astronomer Software installation guide](install-airgapped.md) for  comprehensive instructions showing how to set up a full installation. Or check out [Upgrade Astronomer](upgrade-astronomer.md) for steps to upgrade an existing installation.

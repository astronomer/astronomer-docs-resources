...
2. Change the Docker image in the `FROM` statement of your `Dockerfile` to a new version of Astro Runtime. For example, to upgrade to the latest version of Runtime, you would change the `FROM` statement in your Dockerfile to:
    ...
    ```dockerfile
    FROM quay.io/astronomer/astro-runtime:{{2.x_RUNTIME_VER}}
    ```
  ...
  <Error>After you upgrade your Airflow version, you can't revert to an earlier version.
...
Canceling the Airflow upgrade process does not interrupt or otherwise impact your Airflow Deployment or code that's running.</Error>info
...
<Info>If an Astro Runtime version isn't included in this section, then there are no specific upgrade considerations for that version.</Info>

#### Runtime 12 (Airflow 2.10)

##### Upgrade to Python 3.12

Starting with Python version 3.12, Python has removed the module [`imp`](https://docs.python.org/3.11/library/imp.html). This can cause errors for your DAGs if you use `imp` to access materials using the `import` statement. Python recommends migrating to [`importlib`](https://docs.python.org/3.11/library/importlib.html#module-importlib) instead.
...
##### Incompatibility between Git-Sync and Runtime 12.1.0

Astro Runtime 12.1.0 is incompatible with [Git-Sync](https://airflow.apache.org/docs/helm-chart/stable/manage-dags-files.html#using-git-sync). Upgrade to Astro Runtime 12.1.1 to use Git-Sync with Astronomer Software.
